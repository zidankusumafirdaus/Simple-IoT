...
while True:
    ret, image = cap.read() #cap.read(): Mengambil frame dari kamera. Jika gagal, mencetak pesan error dan keluar dari loop.
    if not ret:
        print("Error: Failed to capture image.")
        break

    image = cv2.cvtColor(cv2.flip(image, 1), cv2.COLOR_BGR2RGB) #cv2.flip(image, 1): Membalik gambar secara horizontal (mirror effect). 
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
            ...
            #Mendapatkan posisi landmark pergelangan tangan (WRIST) untuk menentukan di mana label akan ditampilkan.
            ...
            #Menampilkan label di atas pergelangan tangan dengan posisi yang sudah dihitung.
            # Menggambar landmark tangan pada gambar menggunakan `mp_drawing.draw_landmarks
